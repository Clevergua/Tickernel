...
- (void)mtkView:(MTKView *)view drawableSizeWillChange:(CGSize)size {
    if (self.pTickernelEngine) {
        tickernelUpdate(self.pTickernelEngine, (uint32_t)size.width, (uint32_t)size.height, self.keyCodes);
    }
}
...
- (void)drawInMTKView:(MTKView *)view {
    ...
    if (self.pTickernelEngine) {
        tickernelUpdate(self.pTickernelEngine, (uint32_t)view.drawableSize.width, (uint32_t)view.drawableSize.height, self.keyCodes);
    }
}
