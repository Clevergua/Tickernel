...
static void hasAllRequiredExtensions(GraphicsContext *pGraphicsContext, VkPhysicalDevice vkPhysicalDevice, bool *pHasAllRequiredExtensions)
{
    ...
    uint32_t requiredExtensionCount = TICKERNEL_ARRAY_COUNT(requiredExtensionNames);
...
static void createLogicalDevice(GraphicsContext *pGraphicsContext)
{
    ...
    uint32_t extensionCount = TICKERNEL_ARRAY_COUNT(extensionNames);
...
static void createSwapchain(GraphicsContext *pGraphicsContext, uint32_t swapchainWidth, uint32_t swapchainHeight)
{
    ...
    if (graphicsQueueFamilyIndex != presentQueueFamilyIndex)
    {
        ...
        queueFamilyIndexCount = TICKERNEL_ARRAY_COUNT(queueFamilyIndices);
    }
    else
    {
...
static void resizeDynamicAttachment(GraphicsContext *pGraphicsContext, Attachment *pAttachment)
{
    ...
    // TODO 遍历所有subpass更新其vkDescriptorSet
    for (uint32_t pPipelineIndex = 0; pPipelineIndex < pDynamicAttachmentContent->subpassPtrDynamicArray.count; pPipelineIndex++)
    {
        Subpass *pSubpass = TICKERNEL_GET_FROM_DYNAMIC_ARRAY(&pDynamicAttachmentContent->subpassPtrDynamicArray, pPipelineIndex, Subpass *);
        VkImageLayout vkImageLayout = pSubpass->vkImageLayout;
        uint32_t binding = pSubpass->binding;
        VkWriteDescriptorSet writeDescriptorSet = {
            .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
            .pNext = NULL,
            .dstSet = pSubpass->vkDescriptorSet,
            .dstBinding = binding,
            .dstArrayElement = 0,
            .descriptorCount = 1,
...
static void CreateSpvReflectShaderModule(const char *filePath, SpvReflectShaderModule *pSpvReflectShaderModule)
{
    ...
    tickernelAssert(spvReflectResult == SPV_REFLECT_RESULT_SUCCESS, "Failed to reflect shader module: %s", filePath);
...
static void CollectDescriptorDatas(SpvReflectShaderModule spvReflectShaderModule, TickernelDynamicArray *pVkDescriptorSetLayoutBindingDynamicArray, TickernelDynamicArray *pVkDescriptorPoolSizeDynamicArray, uint32_t descriptorSet)
{
    for (uint32_t setIndex = 0; setIndex < spvReflectShaderModule.descriptor_set_count; setIndex++)
    {
        ...
        if (descriptorSet == spvReflectDescriptorSet.set)
        {
            for (uint32_t bindingIndex = 0; bindingIndex < spvReflectDescriptorSet.binding_count; bindingIndex++)
            {
                ...
                uint32_t addedIndex;
                for (addedIndex = 0; addedIndex < pVkDescriptorSetLayoutBindingDynamicArray->count; addedIndex++)
                {
                    VkDescriptorSetLayoutBinding *pAddedBinding = NULL;
                    tickernelGetFromDynamicArray(pVkDescriptorSetLayoutBindingDynamicArray, addedIndex, (void **)&pAddedBinding);
                ...
                if (addedIndex < pVkDescriptorSetLayoutBindingDynamicArray->count)
                {
                ...
                else
                {
                    ...
                    for (poolSizeIndex = 0; poolSizeIndex < pVkDescriptorPoolSizeDynamicArray->count; poolSizeIndex++)
                    {
                        VkDescriptorPoolSize *pVkDescriptorPoolSize = NULL;
                        tickernelGetFromDynamicArray(pVkDescriptorPoolSizeDynamicArray, poolSizeIndex, (void **)&pVkDescriptorPoolSize);
                    ...
                    if (poolSizeIndex < pVkDescriptorPoolSizeDynamicArray->count)
                    {
                    ...
                    else
                    {
                        ...
                        tickernelAddToDynamicArray(pVkDescriptorPoolSizeDynamicArray, &vkDescriptorPoolSize, pVkDescriptorPoolSizeDynamicArray->count);
                    }
                }
            }
            // Skip other sets.
...
{
    *ppPipeline = tickernelMalloc(sizeof(Pipeline));
    // vkPipelineLayout
    ...
    tickernelCreateDynamicArray(&vkDescriptorSetLayoutBindingDynamicArray, sizeof(VkDescriptorSetLayoutBinding), 1);
    ...
    for (uint32_t stageIndex = 0; stageIndex < stageCount; stageIndex++)
    {
        ...
        CollectDescriptorDatas(spvReflectShaderModule, &vkDescriptorSetLayoutBindingDynamicArray, &vkDescriptorPoolSizeDynamicArray, TICKERNEL_MATERIAL_DESCRIPTOR_SET);
        DestroySpvReflectShaderModule(&spvReflectShaderModules[stageIndex]);
    TODO:
        创建ShaderModule的时候可能有用 ?
    }
    VkDescriptorSetLayout materialVkDescriptorSetLayout;
    ...
    result = vkCreateDescriptorSetLayout(vkDevice, &vkDescriptorSetLayoutCreateInfo, NULL, &materialVkDescriptorSetLayout);
    tryThrowVulkanError(result);

    tickernelFree(spvReflectShaderModules);
    tickernelDestroyDynamicArray(vkDescriptorSetLayoutBindingDynamicArray);
    VkDescriptorSetLayout vkDescriptorSetLayouts[TICKERNEL_MAX_DESCRIPTOR_SET];
    vkDescriptorSetLayouts[TICKERNEL_MATERIAL_DESCRIPTOR_SET] = materialVkDescriptorSetLayout;
    vkDescriptorSetLayouts[TICKERNEL_SUBPASS_DESCRIPTOR_SET] = pRenderPass->subpasses[subpassIndex].vkDescriptorSetLayout;
    vkDescriptorSetLayouts[TICKERNEL_GLOBAL_DESCRIPTOR_SET] = pGraphicsContext->vkDescriptorSetLayout;

    VkPipelineLayoutCreateInfo vkPipelineLayoutCreateInfo = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
        .pNext = NULL,
        .setLayoutCount = TICKERNEL_MAX_DESCRIPTOR_SET,
...
void destroyPipeline(GraphicsContext *pGraphicsContext, RenderPass *pRenderPass, uint32_t subpassIndex, Pipeline *pPipeline)
{
    ...
    vkDestroyDescriptorSetLayout(vkDevice, pPipeline->vkDescriptorSetLayout, NULL);
    // TODO
...
void createRenderPass(GraphicsContext *pGraphicsContext, uint32_t attachmentCount, VkAttachmentDescription *vkAttachmentDescriptions, Attachment **pAttachments, uint32_t subpassCount, VkSubpassDescription *vkSubpassDescriptions, TickernelDynamicArray *spvPathDynamicArrays, uint32_t vkSubpassDependencyCount, VkSubpassDependency *vkSubpassDependencies, uint32_t renderPassIndex, RenderPass **ppRenderPass)
{
    ...
    for (uint32_t subpassIndex = 0; subpassIndex < pRenderPass->subpassCount; subpassIndex++)
    {
        ...
        for (uint32_t pathIndex = 0; pathIndex < pathDynamicArray.count; pathIndex++)
        {
            ...
            CollectDescriptorDatas(spvReflectShaderModule, &vkDescriptorSetLayoutBindingDynamicArray, &vkDescriptorPoolSizeDynamicArray, TICKERNEL_SUBPASS_DESCRIPTOR_SET);
...
void destroyRenderPass(GraphicsContext *pGraphicsContext, RenderPass *pRenderPass)
{
    ...
    for (uint32_t i = 0; i < pRenderPass->subpassCount; i++)
    {
        ...
        result = vkFreeDescriptorSets(vkDevice, pSubpass->vkDescriptorPool, 1, &pSubpass->vkDescriptorSet);
        tryThrowVulkanError(result);
